...
public class RsResponse {
	...
	/**
	 * 创建WebApplicationException, 使用标准状态码与自定义信息并记录错误信息.
	 * @param status
	 * @param content
	 * @return
	 */
	public static WebApplicationException buildJsonException(Status status, Object content) {
		return new WebApplicationException(buildJsonResponse(status, content));
	}
